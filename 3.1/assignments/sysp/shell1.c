...
void tokenise_redirect_input_output(char *cmd_exec)
{
  ...
  input_file_redirection = strdup(io_token[1]);
  output_file_redirection = strdup(io_token[2]);
...
void tokenise_redirect_input(char *cmd_exec)
{
  ...
  input_file_redirection = strdup(i_token[1]);
...
void tokenise_redirect_output(char *cmd_exec)
{
  ...
  output_file_redirection = strdup(o_token[1]);
...
static int command(int input, int first, int last, char *cmd_exec)
{
  ...
  if (pid == 0)
  {
    ...
    {
      output_fd = creat(output_file_redirection, 0644);
      if (output_fd < 0)
      {
        fprintf(stderr, "Failed to open %s for writing\n", output_file_redirection);
        return (EXIT_FAILURE);
      }
      dup2(output_fd, 1);
      close(output_fd);
    ...
    {
      input_fd = open(input_file_redirection, O_RDONLY, 0);
      if (input_fd < 0)
      {
        fprintf(stderr, "Failed to open %s for reading\n", input_file_redirection);
        return (EXIT_FAILURE);
      }
      dup2(input_fd, 0);
      close(input_fd);
